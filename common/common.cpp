...
void Common::moveToCenter(QWidget *tmp)
{
     ...
     // 移动窗口
     tmp->move((screenRect.width() - tmp->width())/2, (screenRect.height() - tmp->height())/2);
}
...
{
//    QStringList fileNames = QFileDialog::getOpenFileNames(new QWidget(), tr("Open Files"), QDir::homePath(), tr("Text Files (*.json)"));
//    foreach (const QString &fileName, fileNames) {
//        QString relativePath = QDir::current().relativeFilePath(fileName);
//        qDebug() << "Selected file path: " << fileName.toLocal8Bit();
//        qDebug() << "Selected file relative path: " << relativePath.toLocal8Bit();
//    }
    QFile file(path);
    ...
    if (!file.open(QIODevice::ReadOnly)) {
        // 打开失败
        qDebug() << "Failed to open file: " << path<<" "<< file.error() << ", error message: " << file.errorString().toLocal8Bit();
        return "";
    }
...
void Common::writeLoginInfo(QString user, QString pwd, bool isRemeber, QString path)
{
    ...
    {
        qDebug() << " QJsonDocument::fromVariant(json) err";
        return;
    }
...
void Common::writeRecord(QString user, QString name, QString code, QString path)
{
    ...
    qDebug() << "fileName = " << fileName.toUtf8().data();
    QByteArray array;
